...
$(".xuanXiangKa ul li").click(function(){
    ...
    tabSel=index;
    $(".tab_box").eq(index).children("li").show().parent(".tab_box").siblings(".tab_box").children("li").hide(); //在按钮选中时在下面显示相应的内容，同时隐藏不需要的框架内容
    //$("#scroller").css("transform", "translate(0px, -26px)");
    myScroll.refresh();
    $('.yinCang').show();
    $(".caoZuoAnNiu a").eq(index).show().siblings(".tab_box").hide();
});
$('.xuanXiangKa ul li:first-child').click(function () {
    $('.yinCang').hide();
});
...
function Load() {
    getxieyis("seller",js_page,qiehuanType);
    myScroll.refresh();
}
//上拉刷新下拉加载
...
$(".biaoGe .bottom a:not(.xiuGai,.juJue,.chaKan,.baojia,.bao,.zengJia)").click(function(){
    $(".zhezhao").show();
    $('.con h5').html($(this).html());
});
//协议名称限制字数
intercept('.biaoGe .middle a',17);
//点击确定的效果
...
$('.sure').click(function () {
    if($('.text').val()==''){
        ...
        $('.zhezhao2').hide();
        //调用拒绝
        vm.caozuoiXieyi(jujue_id,'拒绝',$('.text').val())
    }
...
intercept('.shangPinMingCheng',36);
//查看驳回原因
