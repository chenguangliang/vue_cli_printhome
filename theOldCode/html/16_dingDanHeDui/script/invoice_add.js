        ...
        saveInvoice:function(){
            ...
            if($('.tab_box .div2').css('display')=='block'){
                ...
                if($('#invoice_title').val()==''){
                    $("#invoice_title_errorMsg_normal").text("请输入发票抬头信息");
                    $("#invoice_title").focus();
                    return;
                }else {
                    $("#invoice_title_errorMsg_normal").text("");
                }
                // 校验纳税人识别码
                ...
                /*{ ----用到下面代码，把这个大括号去掉，我是为了好折叠代码
                    // 校验姓名
                    if($('#name1').val()==''){
                        $("#consigneeName_errorMsg1").text("请输入收票人姓名");
                        $("#name1").focus();
                        return;
                    }else {
                        $("#consigneeName_errorMsg1").text("");
                    }

                    // 校验收票人手机
                    if($('#phone1').val()==''){
                        $("#consigneeMobile_errorMsg1").text("请填写收票人手机");
                        $("#phone1").focus();
                        return;
                    }else{
                        var chrnum = /^[1]([3|5|7|8][0-9]{1}|59|58|88|89)[0-9]{8}$/;
                        if(!chrnum.test($('#phone1').val())){
                            $("#consigneeMobile_errorMsg1").text("手机号码格式不正确");
                            $("#phone1").focus();
                            return;
                        }
                        $("#consigneeMobile_errorMsg1").text("");
                    }

                    // 校验省
                    if($('#sel_Province1').val()==''){
                        $("#invoice_province_errorMsg1").text("请选择省");
                        $("#sel_Province1").focus();
                        return;
                    }else{
                        $("#invoice_province_errorMsg1").text("");
                    }

                    // 校验市
                    if($('#sel_City1').val()==''){
                        $("#invoice_province_errorMsg1").text("请选择市");
                        $("#sel_City1").focus();
                        return;
                    }else{
                        $("#invoice_province_errorMsg1").text("");
                    ...
                    // 校验县
                    if($('#sel_County1').val()==''){
                        $("#invoice_province_errorMsg1").text("请选择县");
                        $("#sel_County1").focus();
                        return;
                    }else{
                        $("#invoice_province_errorMsg1").text("");
                    ...
                    // 校验详细地址
                    if($('#detailAddress1').val()==''){
                        $("#detailAddress_errorMsg1").text("请填写详细地址");
                        $("#detailAddress1").focus();
                        return;
                    }else{
                        $("#detailAddress_errorMsg1").text("");
                    }
                }*/
            ...
            var temp = this;
            if(this.invoiceDTO.invoice == 2){
                deleteJSONFiled(temp.submitInfo,"noInvoiceFlag");
                /*var allSubmitData={};
                allSubmitData.orderId=this.submitInfo.orderId;
                allSubmitData.buyerId=this.submitInfo.buyerId;
                allSubmitData.invoice=this.invoiceDTO.invoice;
                allSubmitData.consigneeName=this.invoiceDTO.consigneeName;
                allSubmitData.consigneeMobile=this.invoiceDTO.consigneeMobile;
                allSubmitData.provinceId=this.invoiceDTO.provinceId;
                allSubmitData.cityId=this.invoiceDTO.cityId;
                allSubmitData.countyId=this.invoiceDTO.countyId;
                allSubmitData.detailAddress=this.invoiceDTO.detailAddress;*/
                var allSubmitData={};
                allSubmitData.invoice=this.invoiceDTO.invoice;
                allSubmitData.invoiceTitle=this.invoiceDTO.invoiceTitle;
                allSubmitData.taxpayerCode=this.invoiceDTO.taxpayerCode;
                //补开发票增加的
                allSubmitData.orderId=this.submitInfo.orderId;
                allSubmitData.sellerId=this.submitInfo.sellerId;
                allSubmitData.sign='apply';
                console.log(allSubmitData);
                $.jsonAjax(getUrl('invoice/apply'),allSubmitData,function(data, status, xhr){
                    //如果成功就返回上一页，并且让上一页刷新
                    if(data.success){
                ...
                //补开发票增加的
                this.invoiceDTO.orderId=this.submitInfo.orderId;
                this.invoiceDTO.sellerId=this.submitInfo.sellerId;
                this.invoiceDTO.sign='apply';
                $.jsonAjax(getUrl('invoice/apply'),this.invoiceDTO,function(data, status, xhr){
                    //如果成功就返回上一页，并且让上一页刷新
                    if(data.success){
                        location.replace(document.referrer);
                    }
