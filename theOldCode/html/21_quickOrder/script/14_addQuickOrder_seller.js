...
var quickOrderVue = new Vue({
    ...
    data: {
        ...
        periodDetail:[{periodCounts:1,orderScale:50.0,orderAmount:'',periodPremise:6,periodPremiseDays:0},{periodCounts:2,orderScale:50.0,orderAmount:'',periodPremise:6,periodPremiseDays:0}],
        ...
        invoiceInfo:{
            ...
            countyId:0, detailAddress:'',buyerId:''},//上传发票信息带回来一个Id即可,
        defaultPeriod:{'period':2,'isPrepay':0,'preOAmount':0,'calcuMethod':0,'totalOAmount':0},
        submitInfo: { //key 加单引号的为已处理  双引号为置空不用管
            'modelType':2, 'buyerPhone':'', 'buyerName':'', 'buyerUserId':'', 'invoiceId':'', 'invoiceType':0, 'isAudit':0,
    ...
    methods: {
        ...
        buyerPhoneCheck: function () {
            ...
            $.jsonAjax(getUrl("quickor/user/buyerPhoneCheck"), {phone:temp.selectedSeller.umobile}, function (data, status, xhr) {
                if(data){
                    if(!data.belong){
        ...
        stepOneNext: function () {
            ...
            if(temp.selectedSeller.umobile.indexOf('****')==-1){
                if(!(/^1[34578]\d{9}$/.test(temp.selectedSeller.umobile))){
                    popUp_auto_false(1500,"手机号码格式错误!");
                    return;
                }
...
//上面是校验-----------------------------------------------------------------
        ...
        searchPartner: function () {
            ...
            var inputBl = false;
            if(/^[\u4e00-\u9fa5]/.test(temp.userInput)){
				if(temp.userInput.length > 2) {
					inputBl = true;
				}
			}else{
				if(temp.userInput.length > 5){
					inputBl = true;
				} 
			}
            if(inputBl){
        ...
        liToUserInput: function (e,index) {
            ...
            $.jsonAjax(getUrl("/quickor/user/queryUserInfo"), filterJSONNULL(paramData), function (data, status, xhr) {
                if (data.isSuccess) {
                   ...
                   var  afterBuyerInfo=data.result.user;
                    //把买家身份加密
                    if(afterBuyerInfo.linkMan==null || afterBuyerInfo.linkMan==''){

                    }else{
                        afterBuyerInfo.linkMan=afterBuyerInfo.linkMan.substring(0,1)+"***";
                    }
                    //wangguan(xiugai)
                    if(afterBuyerInfo.userEmail==null || afterBuyerInfo.userEmail==''){

                    }else{
                        afterBuyerInfo.userEmail=afterBuyerInfo.userEmail.replace(afterBuyerInfo.userEmail.substring(4,afterBuyerInfo.userEmail.lastIndexOf("@")),"*****");
                    }
                    //wangguan(xiugai)
                    if(afterBuyerInfo.umobile==null || afterBuyerInfo.umobile==''){

                    }else{
                        afterBuyerInfo.umobile=afterBuyerInfo.umobile.substring(0,3)+"****"+afterBuyerInfo.umobile.substring(7,11);
                    }

                    temp.tempSelected=afterBuyerInfo;
                    console.log(data);
                } else {
                    popUp_auto_false(2500, data.msg);
                }
...
//提交发票，带回来一个发票Id--------------------------------------------------------------
        ...
        getInvoiceId: function (flag) {
            ...
            if(flag==2){
                ...
                temp.invoiceInfo.buyerId=temp.selectedSeller.uid;
                //复制出来一份，点击修改再点击取消，应该把参数变到提交时的那时参数，用来回显
                temp.submitInvoiceInfo=copyObj(temp.invoiceInfo);
...
//下面是订单提交部分-----------------------------------------------------------------
        ...
        resetData: function () {
            ...
            if(periodNum>0 && periodNum<13){
                //先扩大数组用来填请求回来的参数
                temp.periodDetail=[{periodCounts:1,orderScale:50.0,orderAmount:'',periodPremise:6,periodPremiseDays:0},{periodCounts:2,orderScale:50.0,orderAmount:'',periodPremise:6,periodPremiseDays:0}];
