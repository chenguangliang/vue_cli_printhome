...
var quickOrderVue = new Vue({
    ...
    data: {
        ...
        submitCommonInvoice:{},//用来储蓄提交时的参数，用来点击取消回显
    ...
    methods: {
       ...
       /* changePayMethod: function ($event) {
        ...
        initOrderDetile: function () {
            var temp = this;
            var paramData = {'orderId': this.urlObj.orderId, 'modleType': 1};
            $.jsonAjax(getUrl("/quickor/master/orderDetile"), paramData, function (data, status, xhr) {
                if (data.isSuccess) {
                    ...
                    //第一步赋值
                    temp.userInput = temp.sellerJm.companyName;
                    /*temp.selectedSeller.uname = temp.sellerJm.uname;
                    temp.selectedSeller.userEmail = temp.sellerJm.userEmail;
                    temp.selectedSeller.umobile = temp.sellerJm.umobile;*/
                    temp.selectedSeller=temp.sellerJm;//吧sellerJm全赋值给selectedSeller，因为liToUserInput函数就是把点击用户的所有信息赋给了selectedSeller
                    //第二步赋值
                    ...
                    if (data.result.order.payMethod == 2 && data.result.order.payDelay == 1) {//企业支付并且是延期支付
...
//下面是订单提交部分-----------------------------------------------------------------
        ...
        submitOrder: function () {
            ...
            // deleteJSONFiled(temp.submitInfo,"id");//把id删掉
            deleteJSONFiled(temp.submitInfo,"orderId");//把orderId删掉
