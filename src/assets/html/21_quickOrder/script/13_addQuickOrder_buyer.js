...
var quickOrderVue = new Vue({
    ...
    data: {
        ...
        tempIndex:'',//用来存储最后删的是
        getUserInput:'',//用于用户输入的信息，点击确定把 userInput给它
        tempSelected:{},//用于存储选取的对方信息，点击确定才赋值给 selectedSeller
        //用于新增商品
        categoryLevOne: [],
        categoryLevTwo: [],
        categoryLevThree: [],
        ...
        periodDetail:[{periodCounts:1,orderScale:50.0,orderAmount:'',periodPremise:6,periodPremiseDays:0},
        ...
        commonInvoicePara:{invoice:2,invoiceTitle:'',taxpayerCode:''},//上传普通发票信息带回来一个Id即可,
        copyCommonInvoicePara:{invoice:2,invoiceTitle:'',taxpayerCode:''},//复制普通发票提交时的参数，用来点击取消回显
        invoiceInfo:{
            id:'',invoice:3,  invoiceTitle:'', taxpayerCode:'', registeredAddress:'', registeredPhone:'',
            bankName:'', bankAccount:'', businessLicensePicUrl:'', taxRegistrationCertificatePicUrl:'', generalTaxpayerPicUrl:'',
            normalContent:'明细', consigneeName:'', consigneeMobile:'', provinceId:0, cityId:0,
            countyId:0, detailAddress:'',buyerId:''},//上传专用发票信息带回来一个Id即可,
        copyInvoiceInfo:{},//复制专用发票提交时的参数，用来点击取消回显
        defaultPeriod:{'period':2,'isPrepay':0,'preOAmount':0,'calcuMethod':0,'totalOAmount':0},
        submitInfo: { //key 加单引号的为已处理  双引号为置空不用管
            'modelType':1, 'buyerPhone':'', 'buyerName':'', 'buyerUserId':'', 'invoiceId':'', 'invoiceType':0, 'isAudit':0,
            'isPeriod':'', 'period':2, 'isPrepay':'', 'itemDetailsBuffer':'',  "parentOrderId":0, 'payDelay':0,
            'payMethod':2, 'payedAmount':0, 'periodDetailsBuffer':'', 'premise':6, 'premiseDays':0, 'prepayScale':'50',
            'auditerUserId':0, 'sellerName':'', 'sellerPhone':'', 'sellerUserId':'', 'sellerEmail':'', 'totalAmount':0,
            'totalPrepayAmount':0, 'unPayedAmount':0,periodMethod:0 // "orderId":0,
        },
        please:'请输入',
        personalityDTO:{},//个性化表单数据
        sampleElement: '<input type="file" id="licenseInput" name="file" @change="licenseChange()">',
    },
    methods: {
        ...
        cancleAddGoods: function () {
            var temp=this;
            printConfirm('放弃新增商品？', function () {
                $(".addGoodsWin").hide();
                $("#addGoods")[0].reset();
                temp.material.itemName="";
                temp.material.unitPrice="";
                temp.categoryLevTwo.splice(0);
                temp.categoryLevThree.splice(0);
                temp.brandList.splice(0);
                $("#pinlei1,#pinlei2").show();
                $(".otherClass,.otherBrand,.otherUnit,.otherSpeci,.otherchecker").hide();
                $(".noNeedCheck").attr('src','../../img/yes-select.png');
                $(".needCheck").attr('src','../../img/no-select.png');
                $(".noAddToMaterial").attr('src','../../img/yes-select.png').siblings('img').attr('src','../../img/no-select.png');
            })
        },
        tempToSelected: function () {//点击确定才把选取的对方信息展示到第一步
            if(this.userInput==''){
                this.tempSelected={};
            }
            this.getUserInput=this.userInput;
            this.selectedSeller=this.tempSelected;
        },
        //如果改变卖家手机号，要从库里查询是否存在
        sellerPhoneCheck: function () {
            var temp=this;
            if(!isNotBlank(temp.selectedSeller.umobile)){
                popUp_auto_false(1500,'请填写卖方电话');
                return;
            }
            if(!(/^1[34578]\d{9}$/.test(temp.selectedSeller.umobile))){
                popUp_auto_false(1500,"手机号码格式错误!");
                return;
            }
            $.jsonAjax(getUrl("quickor/user/sellerPhoneCheck"), {phone:temp.selectedSeller.umobile}, function (data, status, xhr) {
                if(data){
                    if(!data.belong){
                        popUp_auto_false(1500,"手机号码重复!");
                        temp.selectedSeller.umobile='';
                        $(".sellerPhone").val('');
                        return;
                    }
                }else {
                    popUp_auto_false(1500,"链接超时!");
                }
            }, false);
        },
        //第一步的下一步
        stepOneNext: function () {
            var temp=this;
            if(temp.selectedSeller.uid==temp.userInfo.uid){
                popUp_auto_false(2000,'卖家与买家不能重复!');
                return;
            }
            if(!isNotBlank(temp.userInput)){
                popUp_auto_false(1500,'请填写卖方公司全称');
                return;
            }
            if(!isNotBlank(temp.selectedSeller.umobile)){
                popUp_auto_false(1500,'请填写卖方电话');
        ...
        //延期支付详情 点确定时 校验 0.期数2-12   1.最后一期金额、比例不能为负值  2支付金额、比例不能为空值，  3分期金额之和==金额总数  ，比例之和为100
        periodSubmit: function () {
            var temp=this;
            if(temp.submitInfo.period<2 || temp.submitInfo.period>12){
                popUp_auto_false(2500,'分期期数为2—12');
                return;
            }
            ...
            if ($(".noAdvancePayBtn")[0].src.indexOf('yes-select.png') == -1 || $(".noSeparateBtn")[0].src.indexOf('yes-select.png') == -1) {  //有预付款和有分期必须选择一个才可以判断
                for (var i = 0; i < temp.periodDetail.length; i++) {
                    if (!isNotBlank(temp.periodDetail[i].orderScale)) {
                        popUp_auto_false(2500, '每一期的比例和金额不能为空');
                        return;
                    }
                    if (!isNotBlank(temp.periodDetail[i].orderAmount)) {
                        popUp_auto_false(2500, '每一期的金额和比例不能为空');
                        return;
                    }
...
//上面是校验-----------------------------------------------------------------
        ...
        searchPartner: function () {
            var temp=this;
            var paramData = {'modelType':2,buyerInput:temp.userInput};
            temp.tempSelected={};
            console.log(paramData);
            if(temp.userInput.length>0){
        ...
        liToUserInput: function (e,index) {
            $("#userSel").val(e.target.innerText);
            this.userInput=e.target.innerText;
            $(e.target).addClass('redBg').siblings('li').removeClass('redBg');
            console.log(e);
            ...
            $.jsonAjax(getUrl("/quickor/user/queryUserInfo"), filterJSONNULL(paramData), function (data, status, xhr) {
                if (data.isSuccess) {
                    // temp.userInput=data.result.user.companyName;
                    temp.tempSelected=data.result.user;//用于存储选取的对方信息，点击确定才赋值给 selectedSeller
                    console.log(data);
                } else {
                    popUp_auto_false(2500, data.msg);
                }
        ...
        searchGoodsList: function () {
            ...
            $.jsonAjax(getUrl("/quickor/item/queryItemSelectList"), filterJSONNULL(temp.queryGoodsParamData), function (data, status, xhr) {
                if (data) {
                    ...
                    if(temp.$data.goodsList.length == 1){
                    	 $("#selectImgId").attr("src", "../../img/yes-select.png")
                    }
                    setTimeout(function () {
        ...
        selectGoodsSure: function () {
            ...
            $(".wrapScrool .selBtn").each(function (item,index) {
                if(this.src.indexOf('no-select') == -1){//将选择的商品有用的信息，push到临时数组里,用来循环
                    var selectedGoods={itemId:$(this).attr('itemid'),itemName:$(this).attr('itemname'),unitPrice:$(this).attr('unitprice'),itemQuantity:'',itemTotalPrice:0,itemFreight:0};
            ...
            $.jsonAjax(getUrl("/quickor/item/submitCreateItemOrMaterials"), paramData, function (data, status, xhr) {
                if (data.isSuccess) {
                    ...
                    window.setTimeout(function () {
                        $(".addGoodsWin").hide();
                        $("#addGoods")[0].reset();//把填过的信息重置
                        $("#pinlei1,#pinlei2").show();
                        $(".otherClass,.otherBrand,.otherUnit,.otherSpeci,.otherchecker").hide();
                        $(".noNeedCheck").attr('src','../../img/yes-select.png');
                        $(".needCheck").attr('src','../../img/no-select.png');
                        $(".noAddToMaterial").attr('src','../../img/yes-select.png').siblings('img').attr('src','../../img/no-select.png');
                        var newGoods={itemId:data.itemId,itemName:temp.material.itemName,unitPrice:temp.material.unitPrice,itemQuantity:'',itemTotalPrice:0,itemFreight:0};
...
//提交发票，带回来一个发票Id--------------------------------------------------------------
        ...
        getInvoiceId: function (flag) {
            ...
            if(flag==1){//普通发票检验
                ...
                copyObjValue(temp.copyCommonInvoicePara,temp.commonInvoicePara);
                $.jsonAjax(getUrl("/invoice/save"), temp.commonInvoicePara, function (data, status, xhr) {
                    if (data.isSuccess) {
                        popUp_auto(2500, data.msg);
            ...
            if(flag==2){
                ...
                $('.addInvoiceWrap').hide();
                $(".editInvoice").show();
                var invoicePara=temp.invoiceInfo;
                //复制出来一份，点击修改再点击取消，应该把参数变到提交时的那时参数，用来回显
                copyObjValue(temp.copyInvoiceInfo,temp.invoiceInfo);
                $.jsonAjax(getUrl("/invoice/save"), invoicePara, function (data, status, xhr) {
                    if (data.isSuccess) {
                        popUp_auto(2500, data.msg);
                        temp.submitInfo.invoiceId=data.invoiceId;
                    } else {
        ...
        cancelCommonInvoiceInfo: function () {
            var temp=this;
             copyObjValue(temp.commonInvoicePara,temp.copyCommonInvoicePara);
        },
        cancelInvoiceInfo: function () {
            var temp=this;
                copyObjValue(temp.invoiceInfo,temp.copyInvoiceInfo);
            if (temp.copyInvoiceInfo.businessLicensePicUrl) {
                $(".licenseDefault").hide();
                $("#yingYe").css('display', 'inline-block');
                $(".bigLicense")[0].src = $("#yingYe")[0].src = getImgUrl(temp.copyInvoiceInfo.businessLicensePicUrl);
            }else {
                $("#yingYe").css('display', 'none').attr('src','');
                $(".licenseDefault").css('display', 'inline-block');

                //这里应该重置licenseInput，用来再次上传
                $('#licenseInput')[0].value="";
                document.getElementById("licenseInput").value="";
                $("#licenseInput").val('');
                $('#licenseInput').replaceWith($('#licenseInput').clone(true));
                $("#licenceForm")[0].reset();
                console.log($("#licenseInput"));
                /*var a=$("#licenseInput").clone();
                $("#licenseInput").remove();
                a.appendTo($("#licenceForm"));*/
                //Vue.set($("#licenseInput"),'value',"");
            }
            if (temp.copyInvoiceInfo.taxRegistrationCertificatePicUrl) {
                $(".registerDefault").hide();
                $("#shuiWu").css('display', 'inline-block');
                $(".bigRegister")[0].src = $("#shuiWu")[0].src = getImgUrl(temp.copyInvoiceInfo.taxRegistrationCertificatePicUrl);
            }
            else {
                $("#shuiWu").css('display', 'none').attr('src','');
                $(".registerDefault").css('display', 'inline-block');

            }
            if (temp.copyInvoiceInfo.generalTaxpayerPicUrl) {
                $(".proveDefault").hide();
                $("#naShui").css('display', 'inline-block');
                $(".bigProve")[0].src = $("#naShui")[0].src = getImgUrl(temp.copyInvoiceInfo.generalTaxpayerPicUrl);
            }
            else {
                $("#naShui").css('display', 'none').attr('src','');
                $(".proveDefault").css('display', 'inline-block');
            }
            },
...
//下面是订单提交部分-----------------------------------------------------------------
        ...
        submitOrder: function () {
            var temp=this;
            //把买家信息赋给参数
            temp.submitInfo.buyerPhone=temp.userInfo.umobile;
            temp.submitInfo.buyerName=temp.userInfo.companyName;
            temp.submitInfo.buyerUserId=temp.userInfo.uid;
            //支付方式
            ...
            //把卖方信息赋给参数
            temp.submitInfo.sellerPhone=temp.selectedSeller.umobile;
            if(temp.selectedSeller.companyName){
                temp.submitInfo.sellerName=temp.selectedSeller.companyName;
            }else {
                temp.submitInfo.sellerName=temp.userInput;
            }

            temp.submitInfo.sellerUserId=temp.selectedSeller.uid;
            temp.submitInfo.sellerEmail=temp.selectedSeller.userEmail;
            ...
            $.jsonAjax(getUrl("/quickor/master/submitOrder"), orderPara, function (data, status, xhr) {
                if(data.isSuccess){
                    ...
                    setTimeout(function () {
                         window.location.href='./1_quickOrderList_buyer.html';
                    },1500);
                }else {
        ...
        advancePayBtn: function () {this.prePayFlag=1;this.defaultComputePeriod();},
        //切换prePayFlag 无预付款
        noAdvancePayBtn: function () {this.prePayFlag=0;},
        //切换periodFlag 有分期  并计算默认分期
        separateBtn: function () {this.periodFlag=1;this.defaultComputePeriod();},
        //切换periodFlag 无分期
        ...
        percentBtn: function () {this.computeFlag=0;},
        //切换 computeFlag:1  //0 按比例  1按金额
        moneyBtn: function () {this.computeFlag=1;},
        //计算默认分期
    ...
    beforeMount: function () {
        ...
        this.invoiceInfo.buyerId=this.userInfo.uid;
        this.commonInvoicePara.buyerId=this.userInfo.uid;
    ...
    watch:{
        ...
        //这里需要盯着总价,让分期信息跟着动，防止:总价变了，而分期还是原来的金额
        ...
        'submitInfo.payDelay': function (newvalue , oldvalue) {
           ...
           /* if(newvalue==0){  //如果改为立即支付了，之前规定的 延期支付前提为‘收到发票后’，不开发票就不能选了 解开
             if($("#invoiceSel option").eq(0).attr('disabled')){
             $("#invoiceSel option").eq(0).removeAttr('disabled');
             }
             }*/
        },
...
function numSub(num1, num2) {//解决0.3-0.1 不等于 0.2 的问题
